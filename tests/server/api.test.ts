...
const mockOAuthTokenExchange = async (platform: string, code: string, codeVerifier?: string) => {
...
const mockLinkedInPost = async (content: string, accessToken: string) => {
...
const mockTwitterPost = async (content: string, accessToken: string, replyToId?: string) => {
...
describe('Server API Endpoints', () => {
  ...
  describe('POST /api/oauth/token', () => {
    it('should exchange LinkedIn authorization code for access token', async () => {
      ...
      const response = await mockOAuthTokenExchange(
      ...
      expect(response.data.token_type).toBe('Bearer');
             expect((response.data.userProfile as any).id).toBe('linkedin_user_123');
    });
    ...
    it('should exchange Twitter authorization code for access token with PKCE', async () => {
      ...
      const response = await mockOAuthTokenExchange(
        requestBody.platform,
        requestBody.code,
        requestBody.codeVerifier
      );

      expect(response.success).toBe(true);
      expect(response.data.access_token).toBe('twitter_access_token_12345');
      expect(response.data.token_type).toBe('bearer');
             expect((response.data.userProfile as any).data.id).toBe('twitter_user_456');
    });
    ...
    it('should handle Twitter PKCE verification failure', async () => {
      ...
      await expect(mockOAuthTokenExchange(
        requestBody.platform,
        requestBody.code,
        requestBody.codeVerifier
  ...
  describe('POST /api/linkedin/post', () => {
    it('should successfully post to LinkedIn', async () => {
      ...
      const response = await mockLinkedInPost(
        requestBody.content,
  ...
  describe('POST /api/twitter/post', () => {
    it('should successfully post to Twitter', async () => {
      ...
      const response = await mockTwitterPost(
        requestBody.content,
        requestBody.accessToken
      );
    ...
    it('should handle malformed request bodies', () => {
      const malformedRequests = [
        ...
        { content: '' }, // Empty content
             ...
             malformedRequests.forEach((req: any) => {
